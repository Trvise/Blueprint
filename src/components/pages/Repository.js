...
import { uploadFileToFirebase } from './createsteps helpers/CreateStepsUtils';
import { repositoryStyles } from './Repository.styles';
...
const Repository = () => {
    ...
    const editTool = async () => {
        ...
        try {
            ...
            console.log('Updating tool with URL:', updateUrl);
            console.log('Update payload:', updatePayload);
            console.log('Tool ID from editingItem:', editingItem.tool_id);
            ...
            if (response.ok) {
                ...
                console.log('Successfully updated tool:', updatedData);
                await fetchTools();
                ...
                const errorText = await response.text();
                console.error('Failed to edit tool:', response.status, response.statusText);
    ...
    const editMaterial = async () => {
        ...
        try {
            ...
            console.log('Updating material with URL:', updateUrl);
            console.log('Update payload:', updatePayload);
            console.log('Material ID from editingItem:', editingItem.material_id);
            ...
            if (response.ok) {
                const updatedData = await response.json();
                console.log('Successfully updated material:', updatedData);
                await fetchMaterials();
                setShowEditModal(false);
                setEditingItem(null);
                setError(null);
                ...
                const errorText = await response.text();
                console.error('Failed to edit material:', response.status, response.statusText);
                console.error('Error response body:', errorText);
    ...
    const handleEditItem = (item) => {
        setEditingItem({
            ...item,
            imageFile: null // Reset file input for new upload
        });
